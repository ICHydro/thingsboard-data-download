...
#          Alessandro Bortoletto (LINKS Foundation)
# Date   : January 2021

# A collection of functions to perform operations with
# ThingsBoard DB via RESTful API or MQTT
...
def getData(ip, port, user, password, deviceId, key, startTs, endTs):
    """Returns a timeseries with the response to the HTTP RESTful request
    ...
    :type deviceId: str
    :param key: key of the timeseries to read
    :type key: str
    :param startTs: first unix timestamp to fetch
    :type startTs: str or int
    :param endTs: last unix timestamp to fetch
    :type endTs: str or int
    
    :return: Timeseries data for the requested key 
    :rtype: dict

    .. note:: The function reads all the database entries associated 
    with the device and key, with a LIMIT of 200000 and no aggregation
    """
    ...
    try:
        response = requests.post('http://'+ip+':'+str(port)+'/api/auth/login', headers=headersToken, data=data)
        X_AUTH_TOKEN = response.json()['token']
        #print("Codice autorizzazione: ",X_AUTH_TOKEN)
    except Exception as e:
        print("\nAn exception occurred while trying to obtain the authorization from SOURCE Thingsboard: ", e)
        sys.exit(1)
    ...
    try:
        r = requests.get("http://"+ip+":"+str(port)+"/api/plugins/telemetry/DEVICE/"+deviceId+"/values/timeseries?interval="+INTERVAL+"&limit="+LIMIT+"&agg="+AGG+"&keys="+key+"&startTs="+str(startTs)+"&endTs="+str(endTs),headers=headers)
